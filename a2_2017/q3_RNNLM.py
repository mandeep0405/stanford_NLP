...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      #print('place:',self.input_placeholder.get_shape())
      inputs = tf.split(window,num_or_size_splits=self.config.num_steps,axis=1)
      for i in range(len(inputs)):
          inputs[i] = tf.squeeze(inputs[i], [1])
      #print('len:',len(inputs))
      print('shape',inputs[0].get_shape())
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection') :
        U = tf.get_variable("Output_Weight", [self.config.hidden_size,len(self.vocab)],
                            initializer=tf.random_normal_initializer())
        b_2 = tf.get_variable("bias2", [len(self.vocab)],
  ...
  def add_training_op(self, loss):
    ...
    with tf.variable_scope("Optimizer",reuse=None) as scope :
        print("inside adam")
        train_op = tf.train.AdamOptimizer(self.config.lr,name="adam").minimize(loss)
    ### END YOUR CODE
    return train_op
  ...
  def __init__(self, config):
    ...
    print('shape:',self.encoded_train.shape)
    #print('str',self.encoded_train[:200])
  ...
  def add_model(self, inputs):
    ...
    """
    ### YOUR CODE HERE
    with tf.variable_scope('RNNDropoutInput'):
        inputs = [tf.nn.dropout(x, self.config.dropout) for x in inputs]

    with tf.variable_scope("RNN") as scope :
        self.initial_state = tf.zeros([self.config.batch_size,self.config.hidden_size])
        outputs = []
        prev_hidden_state= self.initial_state 
        for t in range(self.config.num_steps):
            if t> 0:
                scope.reuse_variables()

            H = tf.get_variable("hidden", (self.config.hidden_size,self.config.hidden_size))
            I = tf.get_variable("W", (self.config.embed_size,self.config.hidden_size))
#                            initializer=tf.random_normal_initializer())
            b_1 = tf.get_variable("b1", (self.config.hidden_size,))
#                            initializer=tf.constant_initializer(0.0))
    
    
            prev_hidden_state = tf.nn.sigmoid(tf.matmul(prev_hidden_state,H)+ tf.matmul(inputs[t],I)+b_1)
            outputs.append(prev_hidden_state)
        self.final_state = outputs[-1] 
        
    with tf.variable_scope('RNNDropoutOutput'):
        outputs = [tf.nn.dropout(x, self.config.dropout) for x in outputs]
           # outputs[t] = tf.nn.dropout(outputs[t],self.config.dropout)
