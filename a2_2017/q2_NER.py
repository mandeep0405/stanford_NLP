...
class NERModel(LanguageModel):
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):
        W = tf.get_variable("weights1", (self.config.window_size*self.config.embed_size,self.config.hidden_size),
    ...
    with tf.variable_scope("Softmax"):
        U = tf.get_variable("weights2", (self.config.hidden_size,self.config.label_size),
                            initializer=tf.random_normal_initializer())
    ...
    output = tf.nn.dropout(y,self.config.dropout)
    
    reg_loss  = self.config.l2*tf.nn.l2_loss(W) + self.config.l2*tf.nn.l2_loss(U)
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    reg_loss = tf.get_collection('reg_loss')
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=y))+reg_loss
...
def test_predict():
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session() as session:
            session.run(init)
            print('dev:',model.y_dev[:100])
            print('test:',model.y_test[:100])
            print('sum:',np.sum(model.y_dev))
            if np.any(model.y_test):
                print("it is true")
            else:
                print("false")
            saver.restore(session, './weights/ner.weights')
